      ...
      for (let timeslotsIndex = 0, timeslotLen = day.timeslots.length; timeslotsIndex < timeslotLen; timeslotsIndex++) {
        ...
        for (let sessionIndex = 0, sessionsLen = timeslot.sessions.length; sessionIndex < sessionsLen; sessionIndex++) {
          ...
          for (let subSessionIndex = 0, subSessionsLen = timeslot.sessions[sessionIndex].length; subSessionIndex < subSessionsLen; subSessionIndex++) {
            const sessionId = timeslot.sessions[sessionIndex];
            ...
            const startTime = subSessionsLen > 1 && subSessionIndex > 0 ? sessions[timeslot.sessions[sessionIndex]].endTime : timeslot.startTime;
